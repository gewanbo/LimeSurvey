...
function createFieldMap($survey, $style = 'short', $force_refresh = false, $questionid = false, $sLanguage = '', &$aDuplicateQIDs = array())
{
    ...
    foreach ($questions as $arow) {
        ...
        if ($qtypes[$arow['type']]['subquestions'] == 0 && $arow['type'] != "R" && $arow['type'] != "|") {
            ...
            switch ($arow['type']) {
                ...
                case "!":  //DROPDOWN LIST
                    ...
                    break;
                case "M":
                    $abrows = getAnswers($surveyid, $arow['qid'], $sLanguage);
                    foreach ($abrows as $abrow) {
                        $fieldname = "{$arow['sid']}X{$arow['gid']}X{$arow['qid']}{$abrow['code']}";
                        ...
                        if ($style == "full") {
                            $fieldmap[$fieldname]['title'] = $arow['title'];
                            $fieldmap[$fieldname]['question'] = $arow['question'];
                            $fieldmap[$fieldname]['subquestion'] = $abrow['answer'];
                            $fieldmap[$fieldname]['group_name'] = $arow['group_name'];
                            $fieldmap[$fieldname]['mandatory'] = $arow['mandatory'];
                            $fieldmap[$fieldname]['hasconditions'] = $conditions;
                            $fieldmap[$fieldname]['usedinconditions'] = $usedinconditions;
        ...
        elseif ($qtypes[$arow['type']]['subquestions'] == 2 && $qtypes[$arow['type']]['answerscales'] == 0) {
            ...
            unset($answerset);
        } // For multi questions and two scales answers
        elseif ($qtypes[$arow['type']]['subquestions'] == 2 && $qtypes[$arow['type']]['answerscales'] == 2) {
            $abrows = getSubQuestions($surveyid, $arow['qid'], $sLanguage);
            foreach ($abrows as $abrow) {
...
/**
 * Return an array of answers for a given sid/qid
 *
 * @param int $sid
 * @param int $qid
 * @param string $sLanguage Language of the subquestion text
 * @return array
 */
function getAnswers($sid, $qid, $sLanguage)
{
